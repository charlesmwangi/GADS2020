...
class IqRecyclerAdapter(val leaders: List<LearningLeaders>) :
    RecyclerView.Adapter<IqRecyclerAdapter.LeadersViewHolder>() {
    ...
    override fun onBindViewHolder(holder: LeadersViewHolder, position: Int) {
        ...
        holder.score.text =
            leadersList.get(position).score.toString() + " skill IQ score, " + leadersList.get(
                position
            ).country
